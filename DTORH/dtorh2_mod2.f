         ...
         SUBROUTINE DTORH2M2(Z,MDIM,NDIM,MMAX,NMAX,PL,QL,NEWM,NEWN,ERR)
...
C             WRITE(6,*)'MMAX IS TOO LARGE FOR MODE=0'
C             WRITE(6,*)'BETTER TRY MODE=1'
C             STOP
			  ERR=7
			  RETURN
          END IF              
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       ...
       END IF
       IF (ICAL.EQ.0) THEN
C         WRITE(6,*)'YOU MUST CHOOSE MODE=2'
C         STOP
		 ERR=2
		 RETURN
       END IF
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC  
           CALL FRACPSM(QARGU,MMAX+1,0,EPS,TINYSQ,FCP,ERR)
		   IF (ERR.EQ.11) THEN
			 RETURN
		   END IF	
           DD=MMAX+0.5D0
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
        ...
        CALL FRACM(Z,MMAX,0,EPS,TINYSQ,FC,ERR)
		IF (ERR.EQ.11) THEN
			RETURN
		END IF
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
        ...
        CALL FRACM(Z,MMAX+1,0,EPS,TINYSQ,FC2,ERR)
		IF (ERR.EQ.11) THEN
			RETURN
		END IF
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
         CALL FRACM(Z,M,NMAX,EPS,TINYSQ,FC,ERR)
		 IF (ERR.EQ.11) THEN
			RETURN
		END IF
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC 
