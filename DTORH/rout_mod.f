C added error handling through error code by Vassili Saviov
         SUBROUTINE FRACM(Z,M,NMAX,EPS,TINYSQ,FC, ERR) 
         INTEGER M,NMAX,N,MM, ERR          
...
81       D0=B+A*D0
         ...
         END IF
C         IF (MM.EQ.10000) WRITE(*,*)'CF CONVERGENCE FAILS'
		 IF (MM.EQ.10000) THEN 
		     ERR=11
			 RETURN
		 END IF
         ...
         SUBROUTINE FRACPSM(QZ,M,N,EPS,TINYSQ,FC,ERR)
         INTEGER M,N,MM, ERR         
